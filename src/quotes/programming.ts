...
const data: Quotes[] = [
    ...
    },
    {
        ...
        sentences: [
            ...
            "건강한 몸에서 건강한 코드가!",
            "게으름은 개발자의 가장 좋은 친구이다. 자동화 할 수 있는 것들에 시간을 낭비하지 마라.",
            "나는 대단한 프로그래머는 아니다. 그저 좋은 습관들을 가지고있는 평범한 프로그래머일 뿐이다. - Kent Beck",
            "나쁜 코드를 주석처리하고 넘어가버리면 안된다. 코드를 다시 작성하라. - Brian Kernighan",
            ...
            "당신이 어디 사는지 알고있는 사이코패스가 언젠가 당신의 코드를 유지보수하게 될 수도 있다고 생각하면서 코드를 작성하라. - Martin Golding",
            "당신이 잘 하면 할수록 남들은 그 일이 쉬운 일이라고 생각할 것이다.",
            ...
            "동작하지 않는 훌륭한 설계의 코드보다는 그래도 작동하는 미흡한 코드가 낫다.",
            ...
            "바로 투입될 수 있는 눈 앞의 개발자가 뛰어난 능력을 발휘하기를 기다리는 것 보다는,  그냥 뛰어난 개발자에게 여유 시간이 생기기를 기다리는게 낫다. -  Steve McConnell",
            "새로운 기술이 번져가기 시작하면, 최소한 그 배경과 그 기술의 작동 원리를 이해할 필요는 있다. 세부적인 사항은 그 기술이 요구사항을 해결하는 데 도움이 될 때 파악해도 늦지 않다. 오래된 도구로는 새로운 문제를 해결할 수 없을 수 있다. 계속해서 새로운 기술을 익혀나가야 한다.",
            "세상에 불필요한 학습은 없다. 우리는 더 불필요한 것들을 많이 하고 산다.",
            "세상에는 10 가지의 사람이 있다. Binary 를 아는 사람과 모르는 사람.",
            "세상에는 딱 두 가지 프로그래밍 언어가 있다. 사람들이 욕하는 언어와 아무도 사용하지 않는 언어. - Bjarne Stroustrup",
            "소프트웨어 설계를 구성하는 데에는 두 가지 방법이 있다. 첫 번째 방법은 아주 단순하게 만들어서 결함이 없게 만드는 것이다. 또 다른 방법은 결함이 보이지 않을 정도로 아주 복잡하게 만드는 것이다.",
            "소프트웨어를 재사용 가능하게 만들기 위해서는 우선 사용 가능하게부터 만들어야한다. - Ralph Johnson",
            "스스로 사용하고 싶은 것을 만들어라. - 스티브 워즈니악",
            ...
            "오늘 날 프로그래밍은 바보도 아무런 문제없이 쓸 수 있는 거대한 프로그램을 만드려고 애쓰는 개발자와 더 거대하고 더 나은 바보를 만드려는 우주의 경쟁이다. 아직까지는 우주가 이기고 있다. - Rick Cook",
            "올바로 작동하지 않는다고 걱정하지 마라. 만일 모든 게 잘 된다면 굳이 당신이 돈을 받으며 일 할 이유가 없다. - Mosher's Law of Software Engineering",
            "완벽함이란 더 이상 추가할 것이 없을 때 이루어지는 것이 아니라 더 이상 뺄 것이 없을 때 이루어진다. - 에릭 레이몬드",
            "우선 문제를 풀어라. 그 다음 코드를 작성해라. - John Johnson",
            "좋은 프로그래머 대부분은 돈이나 대중에게 받을 찬사를 기대하고 프로그래밍을 하는 것이 아니라 그냥 재미있어서 한다. - Linus Torvalds",
            "좋은 디자인은 그 때문에 소모되는 비용보다 빠르게 가치가 쌓인다. - Thomas C. Gale",
            "좋은 소프트웨어를 만드는 것은 복잡한 기능을 간단하게 보이도록 만드는 것이다. - Grady Booch",
            "처음부터 자신이 문제라는 사실을 인정하면 디버깅이 훨씬 쉬워진다.",
            "최고의 프로그래머들은 그냥 좋은 프로그래머보다 조금 더 나은 사람들이 아니다... 어떤 기준으로 측정해도 10배 이상 더 나은 사람들이다. - Randall E. Stross",
            "컴퓨터 언어를 설계하는 것은 공원을 산책하는 것과 같다. '쥬라기 공원!!!' - Larry Wall",
            "컴퓨터가 이해할 수 있는 코드는 아무나 작성할 수 있다. 좋은 프로그래머는 사람이 이해할 수 있는 코드를 작성한다. - Martin Fowler",
            "컴퓨터는 원래 엄청 빠른데, 프로그래머가 느리게 만들었을 뿐이다.",
            "코드 라인수를 기준으로 프로그램의 진도를 측정하는 것은 비행기 제작 진도를 무게로 측정하는 것과 같다. - Bill Gates",
            "프로그램은 원래 사람이 읽기 위해 작성되는 것이다. 그러다 우연히 컴퓨터에 의해 실행되는것 뿐이다. - Harold Abelson",
            "하드웨어 : 사람이 발로 걷어찰 수 있는 컴퓨터의 한 부분 - Jeff pesis",
            "형편없는 프로그래머가 언어를 탓한다.",
